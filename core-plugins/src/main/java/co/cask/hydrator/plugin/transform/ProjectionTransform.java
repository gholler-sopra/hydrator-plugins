...
public class ProjectionTransform extends Transform<StructuredRecord, StructuredRecord> {
    ...
    public static class ProjectionTransformConfig extends PluginConfig {
        ...
        private ProjectionTransformConfig() {
    ...
    public static class GetSchemaRequest extends ProjectionTransformConfig {
        private Schema inputSchema;
    }

    @Path("getSchema")
    public Schema getSchema(GetSchemaRequest projectionTransformConfig, EndpointPluginContext endpointPluginContext) {
        Schema inputSchema = projectionTransformConfig.inputSchema;
        Set<String> fieldsToDrop = Sets.newHashSet();
        Set<String> fieldsToKeep = Sets.newHashSet();
        BiMap<String, String> fieldsToRename = HashBiMap.create();
        Map<String, Schema.Type> fieldsToConvert = Maps.newHashMap();
        Pattern fieldDelimiter = Pattern.compile("\\s*,\\s*");
        ...
        Schema output = Schema.recordOf(inputSchema.getRecordName() + ".projected", outputFields);
        return output;
    }
